      ...
      <section className="panel hero-panel overflow-hidden p-6 md:p-8">
        <div className="flex flex-col gap-8">
          ...
          <div className="grid gap-4 lg:grid-cols-4">
            ...
            <HeroStat label="Tamanho" value={`${(currentProject.size_bytes / 1024 / 1024).toFixed(2)} MB`} />
            <HeroStat label="Etapas concluídas" value={`${completedStages}/${processStages.length || 1}`} />
          ...
          <div className="card-surface p-5 md:p-6">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold text-slate-950">Pipeline visível para o usuário</h2>
              </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.72fr_1.28fr]">
        <aside className="space-y-6 xl:sticky xl:top-6 xl:self-start">
          <div className="panel p-6 md:p-7">
            ...
            <h2 className="mt-2 text-3xl font-semibold text-slate-950">Configurar e processar</h2>
            <p className="mt-3 text-base leading-7 text-slate-600">
              Ajuste o comportamento do pipeline e dispare uma nova execução sem perder o histórico das saídas anteriores.
            </p>
            ...
            <div className="mt-6 space-y-4">
              ...
              <Toggle
                ...
                detail="Mantém a estratégia de cor quando houver referência confiável."
                checked={payload.color_preferences.preserve_original_colors}
                onChange={(checked) =>
                  setPayload((current) => ({
                    ...current,
                    color_preferences: { ...current.color_preferences, preserve_original_colors: checked },
                  }))
                }
              />
              ...
              <FieldSelect
                ...
                onChange={(value) =>
                  setPayload((current) => ({
                    ...current,
                    orientation_priority: value as ProcessPayload["orientation_priority"],
                  }))
                }
                options={[
              ...
              <Toggle
                label="Hollowing"
                detail="Gera casca interna aproximada com furo de escape quando seguro."
                checked={payload.hollowing_preferences.enabled || payload.hollowing}
                onChange={(checked) =>
                  setPayload((current) => ({
                    ...current,
                    hollowing: checked,
              ...
              <Toggle
                label="Base auxiliar"
                detail="Adiciona base para peças altas ou com pouca área de contato."
                checked={payload.transform_preferences.add_helper_base}
                onChange={(checked) =>
                  setPayload((current) => ({
                    ...current,
                    transform_preferences: { ...current.transform_preferences, add_helper_base: checked },
                  }))
                }
              />
              <Toggle
                label="Simplificar microdetalhes"
                detail="Reduz detalhes minúsculos abaixo da capacidade provável do nozzle."
                checked={payload.transform_preferences.simplify_microdetails}
                onChange={(checked) =>
                  setPayload((current) => ({
                    ...current,
                    transform_preferences: { ...current.transform_preferences, simplify_microdetails: checked },
                  }))
                }
              />
            ...
            >
              {isSubmitting ? "Iniciando..." : "Processar projeto"}
            </button>
          ...
          <div className="panel p-6 md:p-7">
            ...
            <h2 className="mt-2 text-3xl font-semibold text-slate-950">Onde está tudo</h2>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <button
                ...
                className="pill bg-slate-950 text-white"
              >
          ...
          <div className="panel p-6 md:p-7">
            <p className="section-kicker">Bloqueios</p>
            <h2 className="mt-2 text-3xl font-semibold text-slate-950">Perguntas bloqueantes</h2>
            <div className="mt-5 space-y-3">
              {currentProject.blocking_questions.length === 0 ? (
                <div className="card-surface p-5 text-base leading-7 text-slate-600">Nenhum bloqueio formal no momento.</div>
              ) : (
                currentProject.blocking_questions.map((question) => (
                  <div key={question.code} className="rounded-[1.4rem] border border-red-200 bg-red-50 p-5">
                    <p className="text-lg font-semibold text-red-950">{question.question}</p>
                    <p className="mt-3 text-base leading-7 text-red-900/80">{question.reason}</p>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="panel p-6 md:p-7">
            <p className="section-kicker">Perguntas pendentes</p>
            <h2 className="mt-2 text-3xl font-semibold text-slate-950">O que ainda depende de decisão</h2>
            <div className="mt-5 space-y-3">
              {currentProject.questions_pending.length === 0 ? (
                <div className="card-surface p-5 text-base leading-7 text-slate-600">Nenhuma pergunta pendente no momento.</div>
              ) : (
                currentProject.questions_pending.map((question) => (
                  <div key={question.code} className="rounded-[1.4rem] border border-orange-200 bg-orange-50 p-5">
                    <div className="flex items-start justify-between gap-3">
                      <p className="text-lg font-semibold text-orange-950">{question.question}</p>
                      <span className="rounded-full bg-white/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-orange-700">
                        {question.severity}
                      </span>
                    </div>
                    <p className="mt-3 text-base leading-7 text-orange-900/80">{question.reason}</p>
                  </div>
                ))
              )}
            </div>
        ...
        <main className="space-y-6">
          <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
            <SectionCard title="Score de imprimibilidade">
          ...
          <div className="panel p-6 md:p-7">
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-3xl font-semibold text-slate-950">Projeto convertido e ajustado</h2>
              </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
            <div className="panel p-6 md:p-7">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="section-kicker">Preview 3D</p>
                  <h2 className="mt-2 text-3xl font-semibold text-slate-950">Arquivo base</h2>
                </div>
                {previewHref ? (
                  <a href={previewHref} target="_blank" className="pill">
                    Abrir arquivo
                  </a>
                ) : null}
              </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <SectionCard title="Arquivos gerados">
              <ArtifactList items={currentProject.artifacts} empty="Nenhum arquivo gerado." />
            </SectionCard>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <SectionCard title="Bundles e manifesto">
